...
A recipe says "remove the *n*th `s_wait_*` in this kernel's assembly" rather
than naming a line number. Since the assembly under `asm/` is committed and
frozen, either would address it equally well; the ordinal is kept because it
degrades more gracefully when that assembly is regenerated, surviving any edit
that moves code around without changing the number of waits.

### Why the assembly is committed

The compiler decides how many `s_wait_*` a kernel contains, and one mutant is
derived per wait. Compiling at test time would therefore let a toolchain bump
silently reshape the corpus — a different mutant count, different case IDs,
different results — so two runs months apart would not be comparing the same
thing.

Freezing the assembly makes the set of injected defects fixed, and makes moving
it a deliberate, reviewable commit. The cost is that the corpus stops tracking
current codegen: a compiler that starts emitting a new wait pattern is not
exercised until someone runs `regenerate_asm.py`.
...
```
asm/<target>/kernel.s   (COMMITTED, frozen)
     │
     ├── used as-is ───────────────────────> baseline
     │
     └── apply recipe ──> mutant.s
                       ── clang -target amdgcn ──> .o
                       ── clang-offload-bundler ──> .hipfb
                       ── llvm-mc (.incbin) ──> fatbin.o
                       ── hipcc link (+ host.o) ──> executable

kernel.hip ── hipcc -c --cuda-host-only ──> host.o     cached per kernel
```

`host.o` is the one thing still compiled per run. It cannot be committed beside
the assembly: its fatbin symbol is a per-kernel hash (`__hip_fatbin_<hash>`,
read back with `llvm-nm` at embed time) and it is an x86-64 object tied to the
HIP runtime ABI it links against. Embedding under the wrong symbol links
cleanly but leaves the device code unreachable, so the kernel silently does
nothing.

So running a dynamic detector still needs a ROCm install, but a tool that only
*reads* instructions needs nothing beyond a checkout.

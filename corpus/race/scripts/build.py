...
"""
Building a kernel, and a mutant of it, from HIP source down to an executable.
...
edited *assembly* -- there is no source to recompile::
    ...
    HIP source ---- hipcc -S --cuda-device-only ----> device .s
                                                        |
...
The first two steps depend only on the kernel and target, so they are cached
per kernel and shared by all of its mutants; everything downstream is per
mutant.
"""
...
def compile_device_asm(
    ...
    *,
    ...
    """
    ``hipcc -S --cuda-device-only`` -- the assembly every mutant is edited from.
    ...
    _run(argv, f"device assembly for {kernel.name} ({target})")

    if out.is_file():
        return out
    if auto_named.is_file():
        shutil.move(str(auto_named), str(out))
        return out
    raise BuildError(f"hipcc produced neither {out} nor {auto_named}")

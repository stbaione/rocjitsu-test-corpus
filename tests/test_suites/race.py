...
def _ensure_kernel_artifacts(
    ...
    """
    The per-kernel halves: baseline assembly and the host object.

    Both depend only on the kernel and target, so every mutant of a kernel
    shares them. Written via a temporary file and renamed so that concurrent
    workers building sibling mutants cannot observe a half-written artifact.
    """
    cache.mkdir(parents=True, exist_ok=True)
    source = KERNELS_ROOT / f"{kernel_name}.hip"
    asm, host_obj = cache / "baseline.s", cache / "host.o"

    if not asm.is_file():
        staged = cache / f"baseline.s.{os.getpid()}"
        corpus_build.compile_device_asm(source, target, staged, tools=tools)
        staged.replace(asm)
